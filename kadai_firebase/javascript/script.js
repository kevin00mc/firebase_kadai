...
- option -> normal は普通，random は配列状態の text をランダムで一つ投稿する，select は選択肢（個数に限定なし）を提示する
- link: trueでリンク化
*/
...
const chatList = {
    1: {text: 'あなたの目玉をエレキネット！何者なんじゃ!?', continue: true, option: 'normal'},
    2: {text: '「ナンジャモ」です！おはこんハロチャオ！', continue: true, option: 'normal'},
    3: {text: {title: 'Q1', question: '何を知りたい？', choices: ['ナンジャモについて', '尊敬する人について', 'オススメのデザート', '静岡県静岡市清水区HP']}, continue: false, option: 'choices', questionNextSupport: true}, // questionNextSupportは次に質問に対する詳細を投稿するかどうか
    // userCount1：質問
    4: {text: ['https://www.pokemon.co.jp/ex/sv/ja/character/221014_01/', 'https://ja-jp.facebook.com/tomoaki.taniguchi.92/', 'https://www.quil-fait-bon.com/', 'https://www.city.shizuoka.lg.jp/shimizu.html'], continue: true, option: 'normal', link: true},
    5: {text: 'こちらのリンクをクリックしてね！', continue: true, option: 'normal'},
    6: {text: 'あなたのお名前は？', continue: false, option: 'normal'},
    // userCount2：名前
    7: {text: '', continue: true, option: 'normal'},
    8: {text: '今日の体調はいかがですか？', continue: false, option: 'normal'},
    // userCount3：体調
    9: {text: ['そうですか！そんなときはドンナモンジャTVを見るのだ!', 'え!?そんなときはドンナモンジャTVを見るのだ！', 'あ、そう!?取り合えずドンナモンジャTVを見るのだ！'], continue: true, option: 'random'},
    10: {text: ['https://www.youtube.com/watch?v=oy6d1-7Zyw0&ab_channel=%E3%83%9D%E3%82%B1%E3%83%A2%E3%83%B3%E5%85%AC%E5%BC%8FYouTube%E3%83%81%E3%83%A3%E3%83%B3%E3%83%8D%E3%83%AB'], continue: true, option: 'normal',link: true},
    11: {text: '突然だけど、ここで問題！', continue: true, option: 'normal'},
    12: {text: {title: 'Q2', question: 'ナンジャモの相棒は何者なんじゃ!?', choices: ['ハラバリー', 'ガブリアス', 'ニンフィア'], answer: '0'}, continue: false, option: 'choices'},
    // userCount4：ナンジャモの相棒
    13: {text: {qTrue: '', qFalse:'残念！ドンナモンジャTVちゃんと見た？正解は「ハラバリー」でした！'}, continue: true, option: 'normal'},
    14: {text: '', continue: true, option: 'normal'},
    15: {text: {title: '満足度調査', question: 'このナンジャモとのチャットの満足度を5段階で教えてね！（数字が大きいほど満足度が高いよ！）', choices: ['5', '4', '3', '2', '1']}, continue: false, option: 'choices'},
    // userCount5：満足度
    16: {text: 'ありがとう！最後にあなたの感想を聞かせて！', continue: false, option: 'normal'},
    // userCount6：感想
    17: {text: '', continue: false, option: 'normal'}
};


// 投稿文（空の「text」）を加工する関数（中に相手の名前などを挿入する際に用いる）
// userData === ["何を知りたいですか？", 名前","体調","山の質問","満足度","感想"];
// userData はユーザーの回答内容の全てを記憶
function textSpecial() {
    chatList[7].text = `${userData[1]}！おはこんハロチャオ！`;
    chatList[13].text.qTrue = `正解！${userData[1]}、すごい！`;
    chatList[14].text = `${userData[1]}、ありがとう！今日はここで終了！`; 
    chatList[17].text = `${userData[1]}の満足度は「${userData[4]}」，感想は「${userData[5]}」だね！ありがとう！`;
}
